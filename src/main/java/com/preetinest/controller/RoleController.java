...
public class RoleController {
    ...
    @Operation(summary = "Get role by UUID", description = "Retrieve a role by its unique UUID")
    @ApiResponses(value = {
    ...
    public ResponseEntity<Role> getRoleByUuid(@PathVariable String uuid) {
        Optional<Role> role = roleService.getRoleByUuid(uuid);
        return role.map(ResponseEntity::ok)
                .orElseGet(() -> ResponseEntity.notFound().build());
    }

    @Operation(summary = "Create a new role", description = "Create a new role with provided details")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Update a role", description = "Update an existing role by UUID")
    @ApiResponses(value = {
    ...
    @PutMapping("/{uuid}")
    public ResponseEntity<Role> updateRole(@PathVariable String uuid, @RequestBody Role roleDetails) {
        try {
            Role updatedRole = roleService.updateRole(uuid, roleDetails);
